...
//===== Additional Comments: =================================
//= fix everything, hopefully no more bug
//============================================================
...
void status_calc_pc_additional_pre(struct map_session_data **sd, enum e_status_calc_opt *opt) {
	...
	for (i = 0; i < MAX_INVENTORY; ++i) {
		...
		if ((*sd)->inventory_data[i]->equip_script && (*opt & SCO_FIRST) != 0)
			script->run((*sd)->inventory_data[i]->equip_script, 0, (*sd)->bl.id, 0);
	}
	return;
}
...
int pc_delitem_pre(struct map_session_data **sd, int *n, int *amount, int *type, short *reason, e_log_pick_type *log_type) {
	...
	int count = 0;
	for (int i = 0; i < (*sd)->status.inventorySize; ++i)
		if ((*sd)->status.inventory[i].nameid == (*sd)->status.inventory[*n].nameid)
			count += (*sd)->status.inventory[i].amount;
	if (count - *amount == 0) {
		ssd->recalculate = true;
