...
//===== Topic ================================================
//= https://herc.ws/board/topic/7127-maintenance-mode/
//===== Additional Comments: =================================  
//= -- AtCommands --
...
int maintenance_progress(int tid, int64 tick, int id, intptr data) {
	...
	clif->broadcast2(NULL, output, strlen(output) +1, maintenance_color, 0x190, 12, 0, 0, ALL_CLIENT);
	maintenance_group = 0;
...
HPExport void server_online (void) {
	...
	if (SQL->NextRow(map->mysql_handle) == SQL_SUCCESS) {
		char *data;
...
ACMD(maintenanceoff) {
	char esc_name[NAME_LENGTH *2], output[CHAT_SIZE_MAX];
	if (maintenance_endtime <= (int)time(NULL)) {
...
BUILDIN(maintenanceoff) {
	...
	clif->broadcast2(NULL, "Maintenance mode has ended. Players are able to login now.", CHAT_SIZE_MAX, maintenance_color, 0x190, 12, 0, 0, ALL_CLIENT);
	ShowStatus( CL_YELLOW "Maintenance has ended" CL_RESET ".\n" );
	return true;
}
