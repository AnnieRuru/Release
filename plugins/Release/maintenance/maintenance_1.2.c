...
#include "../common/malloc.h" // not again ...
...
int maintenance_progress( int tid, int64 tick, int id, intptr data ) {
	...
	intif->broadcast2( aaa, strlen(aaa) +1, maintenance_color, 0x190, 12, 0, 0);
	maintenance_group = 0;
...
int maintenance_countdown( int tid, int64 tick, int id, intptr data ) {
	...
	if ( countdown > 90 ) {
		safesnprintf( aaa, 255, "Maintenance will start in %d minutes", countdown /60 );
		intif->broadcast2( aaa, strlen(aaa) +1, maintenance_color, 0x190, 12, 0, 0);
		...
		safesnprintf( aaa, 255, "Maintenance will start in %d seconds", countdown );
		intif->broadcast2( aaa, strlen(aaa) +1, maintenance_color, 0x190, 12, 0, 0);
		timer->delete( maintenance_countid, maintenance_countdown );
		...
		TBL_PC *sd;
		safesnprintf( aaa, 255, "Maintenance starts now. Every player will be kick out." );
		intif->broadcast2( aaa, strlen(aaa) +1, maintenance_color, 0x190, 12, 0, 0);
...
HPExport void server_online (void) {
	...
	else if ( maintenance_endtime > time(NULL) ) {
...
bool pc_authok_pre( struct map_session_data *sd, int *login_id2, time_t *expiration_time, int *group_id, struct mmo_charstatus *st, bool *changing_mapservers ) {
	if ( time(NULL) > maintenance_starttime && maintenance_endtime > time(NULL) && *group_id < maintenance_group ) {
		clif->authfail_fd( sd->fd, 1 );
		hookStop();
		return false;
	}
...
void clif_parse_LoadEndAck_pre( int *fd, struct map_session_data *sd ) {
	if ( sd->state.connect_new ) {
		if ( maintenance_starttime > time(NULL) ) {
			char aaa[255];
			safesnprintf( aaa, 255, "Maintenance starts in %d min %d sec, last %d minutes. Server up by %s", ( maintenance_starttime - (int)time(NULL) ) /60, ( maintenance_starttime - (int)time(NULL) ) %60, ( maintenance_endtime - maintenance_starttime ) /60, maintenance_timeformat );
			clif->message( sd->fd, aaa );
		}
		else if ( maintenance_endtime > time(NULL) ) {
			char aaa[255];
			safesnprintf( aaa, 255, "Server is currently in maintenance mode, will end in %d min %d sec on %s", ( maintenance_endtime - (int)time(NULL) ) /60, ( maintenance_endtime - (int)time(NULL) ) %60, maintenance_timeformat );
			clif->message( sd->fd, aaa );
		}
	}
	return;
...
ACMD(maintenance) {
	...
	if ( maintenance_endtime > time(NULL) ) {
		...
		int countdown = maintenance_endtime - (int)time(NULL);
		clif->message( sd->fd, "Type '@maintenanceoff' to turn off maintenance mode." );
		safesnprintf( aaa, 255, "Server is currently in maintenance mode, will end in %d min %d sec on %s", countdown /60, countdown %60, maintenance_timeformat );
		clif->message( sd->fd, aaa );
		return true;
	}
	if ( !message || !*message ) {
...
//	now has to do the stupid string calculation <_<
	...
	if ( group_id < 1 || group_id > 99 ) {
		char aaa[255];
		safesnprintf( aaa, 255, "Invalid Group ID %d. Range must between 1~99.", group_id );
		clif->message( sd->fd, aaa );
		return false;
	}
	if ( kick_duration <= 0 ) {
...
ACMD(maintenanceoff) {
	...
	{
		char aaa[255];
		safesnprintf( aaa, 255, "%s ended the Maintenance mode. Players are able to login now.", sd->status.name );
		intif->broadcast2( aaa, strlen(aaa) +1, maintenance_color, 0x190, 12, 0, 0);
	}
