...
//===== By: ==================================================
//= AnnieRuru
//= Credit - Dastgir -> http://herc.ws/board/topic/7188-market/
//===== Current Version: =====================================
//= 1.9
//===== Compatible With: ===================================== 
//= Hercules 2020-09-20
...
//===== Topic ================================================
//= http://herc.ws/board/topic/7242-market-clone/
//===== Additional Comments: =================================  
//= -- AtCommands --
...
int mob_clone_spawn_market( struct map_session_data *sd, int16 m, int16 x, int16 y, char market_title[], char market_msg[], int msg_color ) { //Copy of mob_clone_spawn with some modification.
	...
	if ( class_ >= MOB_CLONE_END )
		return 0;
	struct mob_db *db = mob->db_data[class_] = (struct mob_db*)aCalloc(1, sizeof(struct mob_db));
	...
	struct chat_data *cd = chat->create(&md->bl, market_title, "", 1, false, 0, "", 0, 1, MAX_LEVEL);
	if ( cd == NULL )
		return 0;
	struct player_data *ssd = getFromMSD(sd, 0);
	ssd->market_chat_id = cd->bl.id;
	...
	if ( sd->vd.dead_sit == 2 )
		clif->sitting( &md->bl );
	if ( market_clone_companion == true ) {
		if ( sd->sc.data[SC_PUSH_CART] ) {
			mmd->market_pushcart = sd->sc.data[SC_PUSH_CART]->val1;
			clif->sc_load( &md->bl, mmd->gid, AREA, status->get_sc_icon(SC_ON_PUSH_CART), mmd->market_pushcart, 0, 0 );
		}
...
void remove_chatroom( int chat_id ) {
	struct chat_data *cd = map->id2cd( chat_id );
	clif->clearchat( cd, 0 );
...
ACMD(marketkill) {
	...
	remove_chatroom( ssd->market_chat_id );
...
int battle_check_target_post( int retVal, struct block_list *src, struct block_list *target, int flag ) {
	if ( retVal == 1 && target->type == BL_MOB ) {
		struct monster_data *mmd = getFromMOBDATA( (struct mob_data*)target, 0 );
		if ( mmd != NULL )
			return -1;
	}
	return retVal;
...
bool chat_joinchat_pre( struct map_session_data **sd, int *chatid, const char **pass ) {
	...
	if ( cd->owner->type == BL_MOB ) {
		struct mob_data *md = (struct mob_data*)cd->owner;
		struct monster_data *mmd = getFromMOBDATA( md, 0 );
		if ( mmd ) {
...
void clif_getareachar_unit_post( struct map_session_data *sd, struct block_list *bl ) {
	if ( bl->type == BL_MOB ) {
		struct monster_data *mmd = getFromMOBDATA( (struct mob_data*)bl, 0);
		if ( mmd != NULL ) {
			clif->dispchat( (struct chat_data*)map->id2bl( mmd->market_chat_id ), sd->fd );
			if ( market_clone_companion == true && mmd->market_pushcart > 0 )
				clif->sc_load( &sd->bl, mmd->gid, SELF, status->get_sc_icon(SC_ON_PUSH_CART), mmd->market_pushcart, 0, 0 );
		}
	}
	return;
}

static void clif_mobname_ack_pre(int *fd, struct block_list **bl) {
	if (*bl == NULL)
		return;
...
int map_quit_pre( struct map_session_data **sd ) {
	struct player_data *ssd = getFromMSD( *sd, 0 );
	if ( ssd != NULL && ssd->market_clone_id ) {
		remove_chatroom( ssd->market_chat_id );
		status_kill( map->id2bl( ssd->market_clone_id ) );
	}
	return 0;
...
int skillnotok_pre( uint16 *skill_id, struct map_session_data **sd ) {
	struct player_data *ssd = getFromMSD( *sd, 0 );
	if ( ssd != NULL && ssd->market_clone_id && ( *skill_id == MC_VENDING || *skill_id == ALL_BUYING_STORE ) ) {
...
HPExport void plugin_init (void) {
	...
	addHookPre( clif, mobname_ack, clif_mobname_ack_pre );
