...
static bool check_filter(int flag, struct map_session_data *sd, struct block_list *tbl, struct map_session_data *tsd) {
	...
	if ((flag & FILTER_CLAN) != 0 && sd->bg_id > 0 && tsd != NULL) {
		if (sd->status.account_id != tsd->status.account_id && sd->status.clan_id == tsd->status.clan_id)
			return true;
	}
	if ((flag & FILTER_OTHER) != 0 && tsd != NULL) {
		if (sd->bg_id > 0 && sd->bg_id != tsd->bg_id) // when you are on a Battleground team, you have to stay on the map anyway ... and your guildmate can go to the other BG team
			return true;
...
static int clif_send_sub_pre(struct block_list **bl, va_list ap) {
	...
	else if (ssd->block_item > 0 && type == AREA && RBUFW(buf, 0) == useItemAckType) {
...
void print_packetfilter_sub(int fd, int flag, const char *message) {
	...
	if ((flag & FILTER_CLAN) != 0) {
		safesnprintf(msg, CHAT_SIZE_MAX, "    -> Clan");
		clif->message(fd, msg);
	}
	if ((flag & FILTER_OTHER) != 0) {
...
void print_packetfilter(int fd, struct player_data *ssd) {
	...
	if (ssd->block_item != 0) {
		safesnprintf(msg, CHAT_SIZE_MAX, "Enabled Item use filter.");
		print_packetfilter_sub(fd, ssd->block_item, msg);
	}
	return;
}
...
void print_packetfilter_guide(int fd) {
	...
	clif->message(fd, "         this doesn't block normal talk from party/guild members.");
...
ACMD(packetfilter) {
	...
	if ((chat_flag & FILTER_SELF) != 0) {
		clif->message(fd, "[C]hat type can't filter [S]elf.");
		clif->message(fd, "Please try again.");
		aFree(tmp);
		return false;
	}
	if ((item_flag & FILTER_SELF) != 0) {
		clif->message(fd, "[I]tem type can't filter [S]elf.");
		clif->message(fd, "Please try again.");
		aFree(tmp);
		return false;
	}
