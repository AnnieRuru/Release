...
int pc_additem_post( int retVal, struct map_session_data *sd, const struct item *item_data, int amount, e_log_pick_type log_type ) {
	...
	pc->setreg( sd, reference_uid( script->add_str("@pickup_uniqueid1"), 0 ), (int)(item_data->unique_id >> 32) );
	pc->setreg( sd, reference_uid( script->add_str("@pickup_uniqueid2"), 0 ), (int)(item_data->unique_id & 0xffffffff) );
