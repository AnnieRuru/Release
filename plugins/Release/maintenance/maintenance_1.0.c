...
int maintenance_countid = 0;
int maintenance_timerid = 0;
...
int maintenance_countdown( int tid, int64 tick, int id, intptr data ) {
	...
	if ( countdown > 90 ) {
		...
		if ( ( countdown % 5 ) > 0 ) // fine tune the timer
			maintenance_countid = timer->add( timer->gettick() + ( ( countdown % 5 ) * 1000 ), maintenance_countdown, 0, 0 );
		else
			maintenance_countid = timer->add( timer->gettick() + 5000, maintenance_countdown, 0, 0 );
	}
	else if ( countdown > 0 ) {
		safesnprintf( aaa, 255, "Maintainance will start in %d seconds", countdown );
		intif->broadcast( aaa , strlen(aaa)+1, 0xFFFF00 );
...
ACMD(maintenanceoff) {
	...
	maintenance_starttime = (int)time(NULL);
	maintenance_endtime = (int)time(NULL);
	if ( maintenance_countid > 0 ) {
		timer->delete( maintenance_countid, maintenance_countdown );
		maintenance_countid = 0;
	}
	if ( maintenance_timerid > 0 ) {
		timer->delete( maintenance_timerid, maintenance_progress );
		maintenance_timerid = 0;
	}
	{
