...
static int pc_inventoryblank_post(int retVal, struct map_session_data *sd) {
	...
	if (retVal == 0 && ssd->allowobbdrop_flag == 1 && ssd->use_item_id == 603)
		return 1; // hack it to tell still has inventory space
	else
		return retVal;
}
...
static int pc_isUseitem_post(int retVal, struct map_session_data *sd, int n) {
	struct player_data *ssd = getFromMSD(sd, 0);
	nullpo_ret(ssd);
	if (retVal == 0 && ssd->allowobbdrop_flag == 1 && ssd->use_item_id == 603 && pc_is90overweight(sd)) {
		ssd->use_item_id = 0;
		return 1; // hack it to bypass the weight restriction
	}
	ssd->use_item_id = 0;
	return retVal;
}
