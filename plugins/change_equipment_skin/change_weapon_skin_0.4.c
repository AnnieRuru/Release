...
struct change_shield_skin_data {
	int itemid;
	int skinid;
};

VECTOR_DECL(struct change_weapon_skin_data) weapon_skin_data;
VECTOR_DECL(struct change_shield_skin_data) shield_skin_data;
...
void clif_sendlook_pre( struct block_list **bl, int *id, int *type, int *val, int *val2, enum send_target *target ) {
	...
	if ( sd != NULL ) {
		...
		if ( *type == LOOK_WEAPON ) {
			...
			i = pc->checkequip( sd, script->equip[EQI_SHADOW_SHIELD -1] );
			if ( i >= 0 ) {
				struct item_data *item = sd->inventory_data[i];
				if ( item == NULL )
					return;
//				ShowDebug( "nameid %d", item->nameid );
//				
					for ( i = 0; i < VECTOR_LENGTH(shield_skin_data); i++ ) {
						ShowDebug( "%d", VECTOR_INDEX(shield_skin_data, i).itemid );
						ShowDebug( "%d\n", VECTOR_INDEX(shield_skin_data, i).skinid );
					}
				
				
				for ( i = 0; i < VECTOR_LENGTH(shield_skin_data); i++ ) {
					if ( item->nameid == VECTOR_INDEX(shield_skin_data, i).itemid ) {
						*val = VECTOR_INDEX(shield_skin_data, i).skinid;
						*type = LOOK_SHIELD;
						break;
					}
				}
...
int read_change_equipment_skin(void) {
	...
	while ( config = libconfig->setting_get_elem( config_db, i++ ) ) {
		...
		else if ( !strcmp( looktype_string, "LOOK_SHIELD" ) ) {
			struct change_shield_skin_data shield_data_;
			shield_data_.itemid = itemid_;
			shield_data_.skinid = skinid_;
			VECTOR_ENSURE( shield_skin_data, 1, 1 );
			VECTOR_PUSH( shield_skin_data, shield_data_ );
		}
		else {
	...
	ShowDebug( "%d\n", VECTOR_LENGTH(shield_skin_data) );
	for ( i = 0; i < VECTOR_LENGTH(shield_skin_data); i++ ) {
		ShowDebug( "%d", VECTOR_INDEX(shield_skin_data, i).itemid );
		ShowDebug( "%d\n", VECTOR_INDEX(shield_skin_data, i).skinid );
	}
	...
	libconfig->destroy( &change_equipment_skin_conf );
	ShowStatus("Done reading '"CL_WHITE"%d"CL_RESET"' entries in '"CL_WHITE"%s"CL_RESET"'.\n", VECTOR_LENGTH(weapon_skin_data) + VECTOR_LENGTH(shield_skin_data), confpath);
	return VECTOR_LENGTH(weapon_skin_data) + VECTOR_LENGTH(shield_skin_data);
}

void clean_change_equipment_skin(void) {
	VECTOR_CLEAR(weapon_skin_data);
	VECTOR_CLEAR(shield_skin_data);
	return;
}
