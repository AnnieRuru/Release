...
static int clif_send_sub_pre(struct block_list **bl, va_list ap) {
	...
	else if (ssd->block_item > 0 && RBUFW(buf, 0) == useItemAckType) { // clif_useitemack
		struct block_list *tbl = map->id2bl(RBUFL(buf, 8));
		struct map_session_data *tsd = target_sd(tbl);
		if (check_filter(ssd->block_item, sd, tbl, tsd))
			hookStop();
	}
	else if (ssd->block_attack > 0 && RBUFW(buf, 0) == damageType) { // clif_damage
	...
	else if (ssd->block_buff > 0 && RBUFW(buf, 0) == 0x9cb && (skill->get_inf(RBUFW(buf, 2)) & (INF_SUPPORT_SKILL|INF_SELF_SKILL)) != 0) { // clif_skill_nodamage
//		ShowDebug("skill ID: %d | target: %d | src: %d\n", RBUFW(buf, 2), RBUFL(buf, 8), RBUFL(buf, 12));
...
void print_packetfilter(int fd, struct player_data *ssd) {
	...
	if (ssd->block_songs != 0)
		print_packetfilter_sub(fd, ssd->block_songs, "Enabled Song filter from bard/dancer.");
	return;
}
...
void print_packetfilter_guide(int fd) {
	...
	clif->message(fd, "    'A' -> block normal Attack animation.");
	clif->message(fd, "    'B' -> block Buff skill/spell effect.");
//	clif->message(fd, "    'T' -> block Target skill/spell effect.");
//	clif->message(fd, "    'G' -> block Ground based spell/aoe effect.");
//	clif->message(fd, "    'S' -> block Song/performance from bard/dancer.");
...
//				break;
	...
	if (!chat_flag && !emotion_flag && !item_flag && !attack_flag && !buff_flag && !target_spell_flag && !aoe_spell_flag && !songs_flag) {
