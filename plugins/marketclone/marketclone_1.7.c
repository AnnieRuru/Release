...
int mob_clone_spawn_market( struct map_session_data *sd, int16 m, int16 x, int16 y, char market_title[], char market_msg[], int msg_color ) { //Copy of mob_clone_spawn with some modification.
	...
	db->option = 0;
	struct mob_data *md = mob->once_spawn_sub( &sd->bl, m, x, y, sd->status.name, class_, "", SZ_SMALL, AI_NONE );
	if ( md == NULL )
		return 0;
	...
	if ( market_clone_companion == true ) {
		if ( sd->sc.data[SC_PUSH_CART] ) {
			...
			clif->sc_load( &md->bl, mmd->gid, AREA, SI_ON_PUSH_CART, mmd->market_pushcart, 0, 0 );
		}
	...
	if ( bl->type == BL_MOB ) {
		...
		if ( mmd != NULL ) {
			...
			if ( market_clone_companion == true && mmd->market_pushcart > 0 )
				clif->sc_load( &sd->bl, mmd->gid, SELF, SI_ON_PUSH_CART, mmd->market_pushcart, 0, 0 );
		}
	}
	return;
}
